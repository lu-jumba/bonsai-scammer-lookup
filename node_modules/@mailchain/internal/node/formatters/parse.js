...
async function parseMimeText(content) {
    ...
    const addressIdentityKeys = parseIdentityKeys(headersMap.get(consts.X_IDENTITY_KEYS) ?? '');
        ...
        if (headerValue != null) {
            ...
            for (const address of parsedAddresses) {
                if (isParsedMailbox(address)) {
                    ...
                    result.push(...address.addresses.map((a) => ({ name: a.name ?? address.name, address: a.address })));
                }
            }
        }
        return result;
...
function parseIdentityKeys(attrStr) {
    ...
    for (const attrPairStr of attrStr.split(';')) {
        ...
        if (attrKey === 'v') {
            ...
            try {
                const identityKey = crypto.publicKeyFromBytes(encoding.decodeHexZeroX(encodedIdentityKey));
                result.set(attrKey, { ...{ identityKey, protocol: protocol } });
            }
            catch (e) {
